...
export default function ClientProjectDetails({ project }: { project: ProjectData }) {
  ...
  return (
    <main ref={containerRef} className="min-h-screen selection:bg-[#D1000A] selection:text-white relative">
      ...
      {/* Progress Bar */}
      <motion.div 
        className="fixed top-0 left-0 right-0 h-1 bg-[#D1000A] z-50 origin-left"
        style={{ scaleX }}
      />
            ...
            <div className="flex flex-wrap gap-3 overflow-hidden">
              {project.tags.map((tag, i) => (
                <motion.span 
                  ...
                  className="text-[10px] font-bold tracking-[0.2em] uppercase border border-white/20 hover:border-[#D1000A] hover:text-[#D1000A] transition-colors rounded-full px-5 py-2.5 backdrop-blur-sm"
                >
                  {tag}
                </motion.span>
              ))}
        ...
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-3 lg:sticky lg:top-[150px] space-y-16">
            <div className="space-y-12">
              <FadeUp delay={0.1}>
                <div className="group">
                  <p className="text-[10px] uppercase tracking-[0.3em] text-[#111]/40 mb-3 group-hover:text-[#D1000A] transition-colors">Client</p>
                  <p className="text-2xl font-medium leading-tight">{project.client}</p>
                </div>
              </FadeUp>
              <FadeUp delay={0.2}>
                <div className="group">
                  <p className="text-[10px] uppercase tracking-[0.3em] text-[#111]/40 mb-3 group-hover:text-[#D1000A] transition-colors">Role</p>
                  <p className="text-2xl font-medium leading-tight">{project.role}</p>
                </div>
              </FadeUp>
              <FadeUp delay={0.3}>
                <div className="group">
                  <p className="text-[10px] uppercase tracking-[0.3em] text-[#111]/40 mb-3 group-hover:text-[#D1000A] transition-colors">Timeline</p>
                  <p className="text-2xl font-medium leading-tight">{project.timeline}</p>
                </div>
              </FadeUp>
              <FadeUp delay={0.4}>
                <div className="group">
                  <p className="text-[10px] uppercase tracking-[0.3em] text-[#111]/40 mb-3 group-hover:text-[#D1000A] transition-colors">Category</p>
            ...
            {project.liveUrl && (
              <FadeUp delay={0.4}>
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-[#D1000A] transform scale-x-0 origin-right transition-transform duration-700 group-hover:scale-x-100" />
                  <style jsx>{`
        ...
        <section ref={nextTargetRef} className="relative h-screen w-full bg-background overflow-hidden flex items-center justify-center group">
          ...
          <Link href={`/work/${project.nextProjectSlug}`} className="relative z-20 flex flex-col items-center max-w-[90vw]">
             <motion.span 
               ...
               className="text-[10px] uppercase tracking-[0.6em] text-white/40 mb-12 transition-all group-hover:tracking-[1em] group-hover:text-[#D1000A]"
             >
               Next Masterpiece
             </motion.span>
             ...
             <div className="mt-12 w-12 h-12 rounded-full border border-white/20 flex items-center justify-center transition-all duration-500 group-hover:scale-125 group-hover:bg-[#D1000A] group-hover:border-[#D1000A]">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="text-white group-hover:text-white group-hover:rotate-45 transition-all">
